...
./jakoolithyprl/set-hypr-profile.sh --dry-run

# Auto-select the profile, link ~/.config/hypr, install the guard,
# and disable PAM faillock's failed-unlock cooldown
...
selected profile, and symlinks:

```text
~/.config/hypr -> ~/projects/scripts/jakoolithyprl/active
```

It also installs a systemd user path unit that watches
`~/.config/hypr/hyprland.conf` and restores the managed wrapper if Hyprland
replaces it with an autogenerated config stub.

The same setup script updates `/etc/security/faillock.conf` with a timestamped
backup so failed Hyprlock attempts do not trigger PAM's default 10-minute
lockout. Use `--skip-faillock` if you want to leave that system file alone.
